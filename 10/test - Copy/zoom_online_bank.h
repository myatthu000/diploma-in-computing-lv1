...
void receive_limit_funcion(int receiver);
void receive_limit_amount(int user_index);
unsigned int calculate_amounts_same_days_for_receiver(int to_calcu_index);

void findR(int to_calcu_index);
...
void transfer_money(int transfer , int receiver , unsigned int amount){
    ...
    if( total_amount>=trans_limit){
        ...
        printf("You cannot transfer beyond limit!\n ");
...
void findR(int to_calcu_index){
    ...
    for(int a=record_counter-1; a>=0 ; a--){
       ...
       for(int aa=0; aa<current_record_counter; aa++){
            if(db[to_calcu_index].trc[aa].note[aa]=='R'){
                printf("\nR\n");
                break;
            }
        }
...
void receive_limit_funcion(int receiver){
    ...
    phone_found = -1;
    finding_phone_number(receiver);
    if(phone_found==-1){
        printf("Phone number does not exit:\n");
    }else{
        int transaction_length_counter = 0;
        printf("Limit receiver's : %s transition limit. ID :%u \nAddress :%s\nTransAmountLimitPerDay :%d : \n",db[phone_found].name,db[phone_found].id,db[phone_found].address,db[phone_found].transAmoLimitPerDay);
        for(int ncc=0; ncc<users ; ncc++){
            if(db[phone_found].id == db[ncc].id){
                printf("User found \n");
                for(int gcc=0; gcc<space_array[ncc]-15 ; gcc++){
                    printf("%s",&db[phone_found].trc[gcc].note[0]);
                    //printf("%d - %s",transaction_length_counter,&db[phone_found].trc[gcc].note[0]);
                    printf("\n");
                    transaction_length_counter++;

                    // loop in transcatation record
                    unsigned int total_amount_for_same_day=0;
                    //int record_counter =space_array[gcc]-15;
                    //printf("-->%d \n",record_counter);
                }
            }
        }
        printf("\n");
        printf("Transcation length counter : %d",transaction_length_counter);
    }
}

void receive_counter(int receiver){
